...
const fortune: FortuneResult = {
  ...
  theme: "最初に決めた条件を増やさない",
  narrative: "今日は判断軸を固定し、時間を区切って楽しむと流れを扱いやすい日です。",
  analysis: {
    assessmentVersion: "v2-foundation-1",
    confidence: "medium",
    consensus: 0.74,
    mainFactors: ["六十干支距離 12/60", "本命数と対象日数の距離 2/9"],
    conflicts: ["引き運では活動性と集中力の判定が分かれている"],
...
describe("LINE UI messages", () => {
  ...
  it("keeps the complete result, reasons, settings, and legal links in reach", () => {
    const serialized = json(fortuneMessage(fortune, baseUrl));
    expect(serialized).toContain("今日のテーマ");
  ...
  it("explains provenance without presenting consensus as win probability", () => {
