  ...
  return page(
    ...
    `
      ...
      <ol>
        ...
        <li><strong>「今日のスロ運」を押す</strong><br>総合スロ運、引き運、相性のよい機種タイプとメーカー、ラッキー要素、立ち回りテーマ、注意ポイントが1枚で届きます。</li>
        <li><strong>必要なら根拠を確認</strong><br>主な要因、鑑定内の参考度・一致度、判断が分かれた点を確認できます。</li>
      </ol>
      <h2>使える操作</h2>
...
export function privacyHtml(): string {
  return page(
    ...
    `
      ...
      <h2>生成AIへの送信</h2>
      <p>鑑定文の生成時、確定済みの総合点、引き運、相性傾向、相性メーカー、ラッキー要素、立ち回りテーマ、注意ポイント、主要因、判断の不一致、内部ルールIDをGemini APIへ送信する場合があります。LINEユーザー識別子、生年月日、出生時刻、サービス秘密値、非表示の内部指標は送信しません。</p>
      <h2>第三者提供</h2>
...
export function termsHtml(): string {
  return page(
    ...
    `
      ...
      <h2>非保証</h2>
      <p>表示内容は、実際の設定、出玉、当選、勝敗、収支、店舗状況その他の事実を予測または保証するものではありません。「鑑定内の参考度」や「鑑定内の一致度」は占い内部の判定状態を示すもので、勝率や的中確率ではありません。</p>
      <h2>メーカー名・商標</h2>
